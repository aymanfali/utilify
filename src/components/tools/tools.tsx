  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="space-y-4">
        ...
        ].map(([name, val, set, min, max]) => (
          <div key={name as string}>
            <div className="mb-1 flex justify-between text-sm">
              <Label>{name as string}</Label>
              <span className="font-mono text-primary">{val as number}</span>
            </div>
            <Slider value={[val as number]} min={min as number} max={max as number} step={1} onValueChange={(v) => (set as (n: number) => void)(v[0])} />
          </div>
        ))}
